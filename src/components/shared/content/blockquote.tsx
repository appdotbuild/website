...
import StackedAvatars from '@/components/pages/blog/stacked-avatars';
...
const blockquoteVariants = cva('blockquote not-prose flex flex-col', {
  variants: {
    variant: {
      default: 'border-l-2 border-muted-foreground py-px pl-6',
      quote: '',
    },
  },
  defaultVariants: {
...
function Blockquote({
  ...
  return (
    ...
    >
      {isThemeQuote && <QuoteIcon className="rotate-180 text-muted-foreground" aria-hidden />}
      <blockquote className={cn(isThemeQuote && 'mt-3.5', isThemeQuote && !centered && 'pl-8')}>
        <p className="text-pretty text-lg font-medium leading-snug tracking-tight md:text-2xl md:leading-normal">
          {quote}
        </p>
      </blockquote>
      {((authorsArray && authorsArray.length > 0) || role) && (
        <figcaption
          className={cn(
            'flex items-center',
            size === 'xs' ? 'mt-3.5 text-[0.8125rem]' : 'mt-5 text-sm',
            isThemeQuote && !centered && 'pl-8',
          )}
        >
          {authorsArray && authorsArray.length > 0 && (
            <>
              <StackedAvatars avatars={avatars} names={names} size={size} />
              <div
                className={cn(
                  'line-clamp-1',
                  avatars.length > 0 && size === 'xs' && 'ml-2.5',
                  avatars.length > 0 && size !== 'xs' && 'ml-3',
                )}
              >
                {names?.map((name, index) => (
                  <span
                    className="mr-1.5 font-medium leading-tight tracking-tight text-secondary-foreground"
                    key={index}
                  >
          ...
          {role && (
            <>
              <span className="line-clamp-1 leading-tight tracking-tight text-muted-foreground">
                —{` `}
                {role}
              </span>
            </>
